...
UZCampusWebMapApp.factory('infoService', function($http, $q, $timeout, $state, $rootScope, APP_CONSTANTS) {
        ...
        var getAllBuildings = function (roomId) {
            var deferred = $q.defer();
            var request = {
                method: 'GET',
                url: APP_CONSTANTS.URI_API + 'estancias/building/',
                contentType: 'application/json',
                dataType: "json"
            };
            $timeout(function () {
                $http(request).then(
                    function (result) {
                        console.log("Success getAllBuildings: ",result.data);
                        deferred.resolve(result.data);
                    },
                    function(err){
                        console.log("Error getAllBuildings: ",err);
                        deferred.reject(err);
                    }
                );
            });
